  ...
  return (
    <PageLayout>
          ...
          <div className="relative">
            ...
            <StaggerContainer className="space-y-8">
              {experience.map((exp) => (
                ...
                >
                  {/* Timeline dot */}
                  <div className="absolute left-4 top-6 w-4 h-4 rounded-full bg-accent border-4 border-[var(--color-bg-primary)] hidden md:block z-10" />

                  {/* Card */}
                  <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-6 sm:p-8 hover:border-accent/25 hover:bg-[var(--color-bg-card-hover)] transition-all duration-300 group">
                    {/* Header */}
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h2 className="font-space text-lg sm:text-xl font-bold text-[var(--color-text-primary)] mb-1 group-hover:text-accent transition-colors">
                            {exp.role}
                          </h2>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-accent font-medium text-sm">{exp.company}</span>
                            <span className="text-[var(--color-text-secondary)]">·</span>
                            <span className="text-[var(--color-text-secondary)] text-sm">{exp.location}</span>
                          </div>
                        </div>
                      </div>
                    ...
                    {/* Achievements */}
                    <div className="mb-6">
                      <p className="text-[var(--color-text-secondary)] text-xs font-mono uppercase tracking-wider mb-4">
                        Key Achievements
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.tech.map((tech) => {
                          ...
                          return (
                            <span
                              key={tech}
                              className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/5 border border-[var(--color-border)] text-[var(--color-text-muted)] text-xs font-mono rounded-lg hover:border-accent/40 hover:text-accent transition-all duration-200"
                            >
      ...
      <section className="py-20 border-t border-[var(--color-border)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <FadeIn>
            <h2 className="font-space text-2xl font-bold text-[var(--color-text-primary)] text-center mb-12">
          ...
          <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {impactMetrics.map(({ metric, desc, Icon }) => (
              <motion.div
                key={metric}
                variants={fadeUpVariant}
                className="p-6 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl text-center hover:border-accent/30 hover:bg-[var(--color-bg-card-hover)] transition-all duration-300 group"
              >
                ...
                <div className="text-[var(--color-text-secondary)] text-sm">{desc}</div>
              </motion.div>
            ))}
          </StaggerContainer>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <FadeIn>
            <div className="bg-gradient-to-br from-accent/10 to-transparent border border-accent/20 rounded-2xl p-8 sm:p-10 text-center">
              <h2 className="font-space text-2xl md:text-3xl font-bold text-[var(--color-text-primary)] mb-4">
