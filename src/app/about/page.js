...
function SkillChip({ skill, SkillIcon, iconColor }) {
  ...
  return (
    <span
      ...
      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white/5 border border-[var(--color-border)] text-[var(--color-text-muted)] text-xs font-mono rounded-lg hover:border-accent/40 hover:text-accent transition-all duration-200 cursor-default"
    >
      {SkillIcon && (
        <SkillIcon
          size={12}
          ...
          <div className="grid md:grid-cols-3 gap-10 md:gap-16">
            ...
            <FadeIn direction="right" className="md:col-span-2">
              <div className="prose prose-invert max-w-none">
                <h2 className="font-space text-2xl font-bold text-[var(--color-text-primary)] mb-6">
                  A bit about me
                </h2>
            ...
            <FadeIn direction="left" delay={0.2}>
              <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-6 h-fit">
                <h3 className="font-space font-semibold text-[var(--color-text-secondary)] mb-5 text-xs tracking-widest uppercase">
                  Quick Info
                </h3>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={item.label} className="flex items-start gap-3">
                      ...
                      <div>
                        <span className="text-[var(--color-text-secondary)] text-xs font-mono uppercase tracking-wider block mb-0.5">
                          {item.label}
                        </span>
                        {item.isEmail ? (
                          <a
                            href={`mailto:${item.value}`}
                            className="text-[var(--color-text-muted)] text-sm hover:text-accent transition-colors break-all"
                          >
                            {item.value}
                          </a>
                        ) : item.isStatus ? (
      ...
      <section className="py-24 border-t border-[var(--color-border)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <FadeIn>
            ...
            <h2 className="font-space text-3xl md:text-4xl font-bold text-[var(--color-text-primary)] mb-14">
              What I work with
            </h2>
          </FadeIn>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h3 className="font-space font-semibold text-[var(--color-text-primary)] text-base">
                      {category}
                    </h3>
                  </div>
          ...
          <StaggerContainer className="space-y-6">
            {education.map((edu) => (
              <motion.div
                key={edu.degree}
                variants={fadeUpVariant}
                className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl hover:border-accent/30 hover:bg-[var(--color-bg-card-hover)] transition-all duration-300 group"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-space font-semibold text-[var(--color-text-primary)] text-base mb-1 group-hover:text-accent transition-colors">
                      {edu.degree}
                    </h3>
                    <p className="text-[var(--color-text-muted)] text-sm">{edu.institution}</p>
                  </div>
                </div>
                <div className="mt-4 md:mt-0 md:text-right pl-[60px] md:pl-0">
                  ...
                  <p className="text-[var(--color-text-secondary)] text-xs mt-1">{edu.duration}</p>
                </div>
              </motion.div>
            ))}
          </StaggerContainer>
          ...
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {certifications.map((cert) => (
              <motion.a
                ...
                variants={fadeUpVariant}
                className="group flex flex-col p-5 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl hover:border-accent/30 hover:bg-[var(--color-bg-card-hover)] transition-all duration-300 hover:-translate-y-1"
              >
                <div className="text-3xl mb-3">{cert.icon}</div>
                <h3 className="font-space font-semibold text-[var(--color-text-primary)] text-sm mb-1.5 group-hover:text-accent transition-colors leading-snug">
                  {cert.title}
                </h3>
                <p className="text-[var(--color-text-secondary)] text-xs mb-3">{cert.issuer}</p>
                <div className="mt-auto pt-3 border-t border-[var(--color-border)] flex items-center justify-between">
                  <span className="text-[var(--color-text-secondary)] text-xs font-mono">{cert.year}</span>
                  <span className="text-accent text-xs font-medium group-hover:underline">View →</span>
                </div>
              </motion.a>
            ))}
          </StaggerContainer>
